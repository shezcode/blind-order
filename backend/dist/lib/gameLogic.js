...
class GameEngine {
    static generateAllPlayerNumbers(config, playerCount) {
        ...
        if (totalNumbers > range) {
            throw new Error(`Not enough unique numbers in range. Need ${totalNumbers}, but range only has ${range} numbers.`);
        }
    ...
    static initializeGame(room) {
        ...
        try {
            ...
            const allPlayerNumbers = this.generateAllPlayerNumbers(config, room.players?.length ?? 0);
    ...
    static validateMove(room, playerId, number) {
    ...
    static getNextExpectedNumber(room) {
    ...
    static makeMove(room, playerId, number) {
    ...
    static addGameEvent(room, event) {
        const gameEvent = {
