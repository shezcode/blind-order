...
import { CreatePlayerData, UpdatePlayerData } from "../lib/types";
...
export class PlayerController {
  ...
  public async updatePlayer(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (updateData.username) {
        const roomPlayers = await this.roomService.getRoomPlayers(
          existingPlayer.roomId
        );
        const usernameExists = roomPlayers.some(
          (p) => p.username === updateData.username && p.id !== playerId
        );
  ...
  public async getPlayersByRoom(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error("Error in getPlayersByRoom:", error);
      res.status(500).json({
        success: false,
